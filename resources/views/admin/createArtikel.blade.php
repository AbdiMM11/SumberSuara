@extends('layouts.layoutAdmin')

@section('content')
    <div class="min-h-screen bg-gradient-to-br from-slate-100 via-slate-50 to-slate-200 px-4 py-10 md:ml-12">
        <div class="max-w-5xl mx-auto">

            <!-- CARD -->
            <div class="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">

                <!-- HEADER -->
                <div class="px-6 md:px-8 py-5 bg-gradient-to-r from-[#1C4E95] via-blue-600 to-indigo-500">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        <div class="flex items-center gap-4">

                            <!-- Ikon Dokumen -->
                            <div class="h-12 w-12 rounded-2xl bg-white/15 flex items-center justify-center shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M7 7h10M7 11h7M7 15h5M6 4h9l3 3v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z" />
                                </svg>
                            </div>

                            <div>
                                <h1 class="text-2xl md:text-3xl font-semibold text-white leading-tight">
                                    Tambah Artikel
                                </h1>
                                <p class="text-sm text-blue-100 mt-1">
                                    Tulis artikel terbaru mengenai kegiatan & karya komunitas Sumber Suara.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- BODY -->
                <div class="px-6 md:px-8 py-6 md:py-8">
                    <form action="{{ route('admin.artikel.store') }}" method="POST" enctype="multipart/form-data"
                        class="space-y-8">
                        @csrf

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                            {{-- JUDUL --}}
                            <div class="space-y-1.5 md:col-span-2">
                                <label for="judul" class="text-sm font-medium text-gray-800">Judul Artikel</label>
                                <div class="relative">
                                    <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center">
                                        <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12h6M9 16h4M5 7h14M6 4h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z" />
                                        </svg>
                                    </span>
                                    <input id="judul" name="judul" type="text" value="{{ old('judul') }}"
                                        placeholder="Contoh: Ruang Kolaborasi Musisi Lokal Lampung"
                                        class="w-full rounded-2xl border border-gray-300 bg-white pl-9 pr-3 py-2.5 text-sm
                                           focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                                </div>
                                @error('judul')
                                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>

                            {{-- AUTHOR --}}
                            <div class="space-y-1.5">
                                <label class="text-sm font-medium text-gray-800">Author</label>
                                <div class="relative">
                                    <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center">
                                        <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5.5 21v-2a4 4 0 0 1 4-4h5a4 4 0 0 1 4 4v2M12 11a4 4 0 1 0-4-4 4 4 0 0 0 4 4z" />
                                        </svg>
                                    </span>
                                    <input id="author" name="author" type="text" value="{{ old('author') }}"
                                        placeholder="Nama penulis"
                                        class="w-full rounded-2xl border border-gray-300 bg-white pl-9 pr-3 py-2.5 text-sm
                                           focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                                </div>
                                @error('author')
                                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>

                            {{-- META TAG --}}
                            <div class="space-y-1.5">
                                <label class="text-sm font-medium text-gray-800">Meta Tag</label>
                                <div class="relative">
                                    <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center">
                                        <svg class="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M7 7h.01M4 4h16v12H4V4zm4 11v3m6-3v3" />
                                        </svg>
                                    </span>
                                    <input id="metatag" name="metatag" type="text" value="{{ old('metatag') }}"
                                        placeholder="Contoh: musik, komunitas, lampung"
                                        class="w-full rounded-2xl border border-gray-300 bg-white pl-9 pr-3 py-2.5 text-sm
                                           focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                                </div>
                            </div>

                            {{-- TANGGAL RILIS --}}
                            <div class="space-y-1.5">
                                <label class="text-sm font-medium text-gray-800">Tanggal Rilis</label>
                                <div class="relative">
                                    <span class="pointer-events-none absolute inset-y-0 left-3 flex items-center">
                                        <svg class="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z" />
                                        </svg>
                                    </span>
                                    <input id="published_at" name="published_at" type="datetime-local"
                                        value="{{ old('published_at') }}"
                                        class="w-full rounded-2xl border border-gray-300 bg-white pl-9 pr-3 py-2.5 text-sm
                                           focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                                </div>
                            </div>

                            {{-- IMAGE --}}
                            <div class="space-y-1.5 md:col-span-2">
                                <label class="text-sm font-medium text-gray-800">Gambar Artikel</label>

                                <label for="gambar"
                                    class="block cursor-pointer rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 px-4 py-4
                                       hover:border-blue-400 hover:bg-blue-50/40 transition flex gap-3">
                                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-white shadow-sm">
                                        <svg class="h-5 w-5 text-blue-600" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M4 16l4.586-4.586a2 2 0 0 1 2.828 0L16 16m-2-2l1.586-1.586a2 2 0 0 1 2.828 0L20 14m-6-10h-4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V8z" />
                                        </svg>
                                    </div>

                                    <div>
                                        <p class="font-medium text-gray-800">Pilih gambar artikel</p>
                                        <p class="text-xs text-gray-500">Format JPG/PNG. Maksimal 2 MB.</p>
                                    </div>
                                </label>

                                <input type="file" id="gambar" name="gambar" class="hidden">

                                @error('gambar')
                                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>

                            {{-- KONTEN --}}
                            <div class="space-y-1.5 md:col-span-2">
                                <label for="konten" class="text-sm font-medium text-gray-800">Isi Artikel</label>
                                <textarea id="konten" name="konten" rows="7" placeholder="Isi artikel..."
                                    class="w-full rounded-2xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm
                                       focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition resize-y">{{ old('konten') }}</textarea>
                                @error('konten')
                                    <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>

                        <!-- BUTTON -->
                        <div class="pt-2 flex justify-end">
                            <button type="submit"
                                class="rounded-2xl bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white shadow-md
                                   hover:bg-blue-500 transition">
                                Simpan Artikel
                            </button>
                        </div>
                    </form>
                </div>

            </div>
        </div>
    </div>
@endsection
